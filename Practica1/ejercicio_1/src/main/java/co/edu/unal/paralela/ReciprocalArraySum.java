...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            // Iterar solamente para el rango asignado a esta tarea
            for (int i = startIndexInclusive; i < endIndexExclusive; i++) {
                partialSum += 1 / input[i];
            }

            this.value = partialSum;
        }
    }
    ...
    protected static double parArraySum(final double[] input) {
        ...
        // Punto medio del arreglo (asumiendo que es divisible por 2)
        int mid = input.length / 2;

        // Crea las dos tareas para ejecutar en paralelo (una para cada mitad del arreglo)
        ...
        // fork() envia la tarea left a otro thread para que se ejecute en paralelo
        left.fork();

        // compute() ejecuta la tarea right en el hilo actual
        right.compute();

        // join() espera a que la tarea left termine y obtiene su resultado
        left.join();
